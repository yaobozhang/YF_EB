...
--STEP1:取订单明细数据（该视图的订单数据已做订单基础过滤）
with t1 as (
			SELECT
				 t."UUID",	   								--明细唯一编码
				 t."STSC_DATE",  								--销售日期
...
--STEP4: 打标“是否糖尿病会员”、“是否胰岛素会员”、“是否诺和诺德会员”	  
	,t1_4 as (  
	select 
	    a.stsc_date,
		a.AT_YEAR,
		...
		a.IS_ND, 
		A.member_id,
		case when b.member_id is not null then 'Y' ELSE 'N' END AS IS_TNB_MEMB, --是否糖尿病会员
		case when c.member_id is not null then 'Y' ELSE 'N' END AS IS_YDS_MEMB, --是否胰岛素会员
		case when d.member_id is not null then 'Y' ELSE 'N' END AS IS_ND_MEMB, --是否诺和诺德会员
		a.sale_amt,
		a.GROS_PROF_AMT
	from t1_3 a
	left join 
...
--step5：同人同天同门店处理	
  ...
  --计算相关指标
	...
	phmc_type_name,
	phmc_code,
	COUNT(DISTINCT CASE WHEN IS_TNB_MEMB='Y' THEN MEMBER_ID end) AS TNB_QTY,
	COUNT(DISTINCT CASE WHEN IS_YDS_MEMB='Y' THEN MEMBER_ID end) AS YDS_QTY,
	COUNT(DISTINCT CASE WHEN IS_ND_MEMB='Y' THEN MEMBER_ID end) AS ND_QTY,
	COUNT(DISTINCT member_id) AS total_QTY,
	sum(sale_times) as sale_times,
	sum(CASE WHEN IS_TNB_MEMB='Y' THEN sale_times end) AS TNB_sale_times,
	sum(CASE WHEN IS_YDS_MEMB='Y' THEN sale_times end) AS YDS_sale_times,
	sum(CASE WHEN IS_ND_MEMB='Y' THEN sale_times end) AS ND_sale_times,
