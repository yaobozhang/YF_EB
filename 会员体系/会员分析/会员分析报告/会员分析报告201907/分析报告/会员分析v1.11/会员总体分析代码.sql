...
--0、数据准备
	--0.1、会员现状口径订单数据：20140101-20190101 ；订单数据基础过滤（积分兑换订单及订金订单、服务性商品及行政赠品、塑料袋）
		/*with t1 as (
			...
			GROUP BY "UUID",								   --明细唯一编码											   
				 ...
				 "MEMBER_ID")                                  --会员编码
		*/
	--0.2、会员分析-年龄性别分析(20180101-20190531)/2.8、会员分析-会员各级别贡献分析(20180601-20190531)/:订单数据基础过滤（积分兑换订单及订金订单、服务性商品及行政赠品、塑料袋）、门店数据过滤（收购加盟门店（目前仅过滤4000的收购门店）、上海医保、当前关停门店、开业时间在20190601后的）；
		with t1 as (
			SELECT
				 t."UUID",	   								--明细唯一编码
				 t."STSC_DATE",  								--销售日期
	...
	,                                
			...
			case when to_char(s.ORDR_SALE_TIME,'hh24')>='06' and to_char(s.ORDR_SALE_TIME,'hh24')<='07' then '6:00-7:59'
				 ...
				 when (to_char(s.ORDR_SALE_TIME,'hh24')>='21' or to_char(s.ORDR_SALE_TIME,'hh24')<='05') then '21:00-5:59' end as sale_hour,	--订单时间段
			s.member_id,					--会员ID
			s.phmc_code,					--门店编码
			s.sale_amt,						--销售金额
			s.GROS_PROF_AMT,				--销售毛利额
			s.PURC_MONEY, 				--营销销售
			s.NO_PURC_MONEY,				--非营销销售
			s.GOODS_CODE,					--商品编码
			s.GOODS_NAME,					--商品名称
			t.phmc_type,					--消费门店类型
			t.ADMS_ORG_CODE,				--消费分公司编码
			t.ADMS_ORG_NAME,				--消费分公司名称
			c.birthday,						--会员生日
			to_char(c.create_time,'YYYY') as MEMB_TEAR,	--会员年份
			to_char(c.create_time,'YYYYMMDD') as MEMB_DAY,	--会员开卡日期
			c.sex,							--会员性别
			C.create_time,					--会员创建日期
			c.come_from,					--会员渠道来源
			n.DICT_NAME,					--门店店型
				...
				(
					  ...
					  ( 
						--上海公司医保店或者开店时间大于20190501或者有关店时间的剔除
						   g1.STAR_BUSI_TIME > '20190601' 		--开业时间在20190601后的过滤掉
						   or 
						   (g1.PHMC_S_NAME like '%医保%' and g1.ADMS_ORG_CODE = '1001' )	--上海医保门店过滤掉
						   or CLOSE_DATE is not null			--关店时间不为空
						   or left(company_code,1)='4'		--4开头过滤掉，即加盟的过滤掉
						   or ADMS_ORG_CODE='1025'		--河北新兴过滤掉
						   or PROP_ATTR in ('Z02','Z07')	--收购过滤掉
					   )
				)
			),
	...
	/*--2.2、会员分析-年新增会员分析*/
		...
		t2_1 as (
			...
			(
				select stsc_date
					...
					,member_id
					,GOODS_NAME
					,DAYS_BETWEEN(MEMB_DAY,stsc_date) AS FIRST_BUY_DAYDIFF		--第一次购买时间间隔
					,Row_Number() OVER(partition by member_id ORDER BY stsc_date Asc) AS ORDER_RANK
				from t1_1 
				where AT_TEAR=MEMB_TEAR
			)where ORDER_RANK=1
		...
		t2_2 as (
			SELECT AT_TEAR
				,GOODS_NAME
				,buy_memb_num
				,Row_Number() OVER(partition by AT_TEAR ORDER BY buy_memb_num DEsc) AS GOODS_RANK
			FROM
			(
				select AT_TEAR
					,GOODS_NAME
					,count(1) as buy_memb_num
				from t2_1
				GROUP BY AT_TEAR
					,GOODS_NAME
			)
		)
		SELECT * FROM T2_2 WHERE GOODS_RANK<=10*/
		...
		t2_3 as (
				select AT_TEAR
					,count(case when ORDER_RANK=2 then member_id)/count(case when ORDER_RANK=1 then member_id) as re_buy_rate	--复购率
				from t2_1
				GROUP BY AT_TEAR
		)
		,
		--统计复购周期分布
		t2_4 as (
				select distinct AT_TEAR,
					PERCENTILE_DISC (0.5) WITHIN GROUP ( ORDER BY day_diff ASC) over(partition by AT_TEAR) as percent_50
				from
				(
					select t1.AT_TEAR
						,t1.member_id
					...
					from t2_1 t1
					left join t2_1 t2
					on t1.AT_TEAR=t2.AT_TEAR
					and t1.member_id=t2.member_id
					and t1.ORDER_RANK=t2.ORDER_RANK+1
				)where day_diff is not null
				group by AT_TEAR
		)
		
		SELECT t1.AT_TEAR,t1.re_buy_rate,t2.percent_50 FROM T2_3 t1
		left join t2_4 t2
		on t1.AT_TEAR=t2.AT_TEAR
		*/
